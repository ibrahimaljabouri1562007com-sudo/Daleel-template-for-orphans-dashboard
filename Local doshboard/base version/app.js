/* ============================================================================
 * لوحة تحليل حالات الأيتام — ENGINE (app.js)
 * Offline, capability-driven analytics for orphan-care case data.
 *
 * Runs AFTER index.html has defined the shared data model + state:
 *   CASE_COLS / VISIT_COLS  – the fixed 48-column categorization + 5 visit cols
 *   PACKAGES                – field bundles the user opts into (identity is core)
 *   FIELD_META              – per-field rules: choice / soft / num / name / date
 *   NAT_CITIES              – Arab-world nationalities → their cities (dependent)
 *   state, BRANDING         – filter/sort/page state; foundation name+logo
 * …and after the vendor libs load: XLSX (SheetJS, read) + ExcelJS (write/format).
 *
 * SECTION MAP (in order below):
 *   1. package picker + onboarding wiring
 *   2. template generator      – ExcelJS mould: dropdowns, numeric/date rules,
 *                                dependent city list (INDIRECT), auto-age formula
 *   3. import + gatekeeper      – validate rows → block / warn / details → build
 *   4. helpers (mapCells, normDate, ageFromDob, pureNum) + search autocomplete
 *   5. Excel write / sync / export report (formatted, ExcelJS) + session persist
 *   6. measures                – KPIs, charts (coverage, vuln, priority, geo,
 *                                income, housing, age, visits, cov-by-city), gov
 *   7. editing                 – add / edit / delete modal + mandatory/soft rules
 *   8. clear-view / clear-data · toolbar · page nav · resume
 *
 * No network. Chrome/Edge only (File System Access API). See README.md/CLAUDE.md.
 * ========================================================================== */
...
/* ---------- branding + onboarding wiring ---------- */
$('#g-logo').addEventListener('change',e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{BRANDING.logo=r.result;$('#g-logo-prev').innerHTML='<img src="'+r.result+'">';};r.readAsDataURL(f);});
$('#g-template').addEventListener('click', downloadTemplate);
$('#g-connect').addEventListener('click', connectExcel);
$('#g-sample').addEventListener('click', ()=>{ BRANDING.name=$('#g-name').value.trim()||'مؤسسة نموذجية'; loadSample(); });
$('#g-file').addEventListener('change', async e=>{ const f=e.target.files[0]; if(!f) return; fileName=f.name; await importBuffer(await f.arrayBuffer()); });
function gateErr(msg){ const b=$('#g-err'); b.innerHTML=msg; b.style.display=msg?'block':'none'; }
...
/* ---------- start ---------- */
function startApp(){
  $('#b-name').textContent=BRANDING.name||'—';
  if(BRANDING.logo) $('#b-logo').innerHTML='<img src="'+BRANDING.logo+'">';
  $('#sync-txt').textContent = fileHandle ? ('متصل: '+fileName) : (fileName||'—');
  $('#sync-dot').className='dot'+(fileHandle?'':' off');
  $('#gate').style.display='none'; $('#app').style.display='block';
  ORIGINAL_DATA=DATA.map(d=>({...d})); ORIGINAL_VISITS=VISITS.map(v=>({...v}));   // snapshot for "revert"
  setDirty(false);                                    // fresh load = no unsaved edits
  const asw=$('#autosave'), asl=$('#autosave-lbl');
  if(asw){ asw.disabled=!fileHandle; if(!fileHandle) asw.checked=false, autoSave=false;
    asl.classList.toggle('off',!fileHandle);
    asl.title = fileHandle ? 'حفظ كل تعديل في ملف Excel المحلي مباشرة' : 'اربط ملفًا محليًا (زر «فتح ملف») لتفعيل الحفظ التلقائي'; }
  state.page=0; buildFilters(); renderAll(); saveSession();
}
/* ---------- session persistence: reopen → straight to the dashboard with the last data ---------- */
const SESSION_KEY='orphans_dashboard_session_v1:'+(location.pathname||'');   // per-file, so the versions don't share sessions
function saveSession(){
  try{ if(fileName==='(بيانات نموذجية)'||!DATA.length) return;   // don't persist the demo/empty
    localStorage.setItem(SESSION_KEY, JSON.stringify({DATA,VISITS,HAS_VISITS,AVAIL:[...AVAIL],name:BRANDING.name,fileName})); }catch(e){}
}
function clearSession(){ try{ localStorage.removeItem(SESSION_KEY); }catch(e){} }
function tryResume(){
  let s=null; try{ s=JSON.parse(localStorage.getItem(SESSION_KEY)||'null'); }catch(e){}
  if(!s||!Array.isArray(s.DATA)||!s.DATA.length) return false;
  DATA=s.DATA; VISITS=s.VISITS||[]; HAS_VISITS=!!s.HAS_VISITS; AVAIL=new Set(s.AVAIL||[]);
  if(s.name && !BRANDING.name) BRANDING.name=s.name;   // keep a fixed brand (e.g. Daleel) if already set
  fileName=s.fileName||'(جلسة محفوظة)'; fileHandle=null;
  derive(); startApp(); toast('استُؤنفت الجلسة السابقة'); return true;
}
...
function buildFilters(){
  ...
  h+=`<button class="reset" id="f-clear">مسح اللوحة</button>`;
  $('#filters').innerHTML=h;
  ...
  $('#f-clear').addEventListener('click', openClr);
...
function saveEditor(force){
  ...
  derive(); closeModal(); renderAll(); rebuildSearchLists(); saveSession(); persistChange(editId?'تم حفظ التعديل':'تمت إضافة الحالة');
}
...
function deleteCase(id){
  ...
  DATA=DATA.filter(d=>d.id!==id); derive(); renderAll(); rebuildSearchLists(); saveSession(); persistChange('تم حذف الحالة');
}

/* ============================ toolbar ============================ */
$('#t-sync').addEventListener('click', async ()=>{ if(fileHandle){ const fl=await fileHandle.getFile(); await importBuffer(await fl.arrayBuffer()); toast('تمت المزامنة من الملف'); } else toast('لا يوجد ملف مرتبط'); });
...
$('#autosave').addEventListener('change', async e=>{ autoSave=e.target.checked; if(autoSave&&fileHandle&&dirty){ const ok=await saveToExcel(true); if(ok){setDirty(false);toast('✓ حُفظت التغييرات في الملف');} } });
/* page navigation: dashboard ⇄ home (gate) */
$('#t-home').addEventListener('click', ()=>{ $('#app').style.display='none'; $('#gate').style.display='block'; const b=$('#g-back'); if(b) b.style.display=DATA.length?'inline-flex':'none'; window.scrollTo(0,0); });
$('#g-back').addEventListener('click', ()=>{ $('#gate').style.display='none'; $('#app').style.display='block'; window.scrollTo(0,0); });
...
function openClr(){ $('#clr-modal').classList.add('on'); }
function closeClr(){ $('#clr-modal').classList.remove('on'); }
$('#clr-x').addEventListener('click', closeClr);
$('#clr-cancel').addEventListener('click', closeClr);
$('#clr-modal').addEventListener('click', e=>{ if(e.target.id==='clr-modal') closeClr(); });
$('#clr-empty').addEventListener('click', ()=>{ DATA=[]; VISITS=[]; HAS_VISITS=false; Object.assign(state,{q:'',tq:'',city:'',priority:'',sp:'',gender:'',page:0}); buildFilters(); renderAll(); setDirty(true); clearSession(); closeClr(); toast('⌫ أُفرغت اللوحة — الملف لم يُمَس'); });
/* on reopen, resume the last session straight to the dashboard (after any brand script has run) */
