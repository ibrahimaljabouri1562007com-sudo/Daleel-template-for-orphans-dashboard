...
/* ============================================================================
 * DATA LAYER — Google Sheet via Apps Script (replaces the local Excel layer)
 *   • JSONP read/write (Apps Script can't send CORS headers)
 *   • sign-in gates the READ by email; role decides view-only vs edit
 *   • edits write straight through to the Sheet (editor/admin only)
 *   • graceful offline/dev fallback to the bundled data/*.json snapshot
 * ========================================================================== */
const API_URL='https://script.google.com/macros/s/AKfycbw73G1dCQLxo_fzB0b6YRVdE0Dftfyzb8wZr13BhVCzzqHQWNZN1VkZFOM8MURiRHarQQ/exec';
let MY_EMAIL=localStorage.getItem('gs_email')||'', MY_ROLE=null, DATA_SRC='sheet';

function apiCall(params){
  return new Promise((resolve,reject)=>{
    const cb='__gscb_'+(apiCall._n=(apiCall._n||0)+1)+'_'+Math.floor(performance.now());
    const s=document.createElement('script'); let done=false;
    const timer=setTimeout(()=>{ if(!done){ cleanup(); reject(new Error('timeout')); } },15000);
    function cleanup(){ done=true; clearTimeout(timer); try{ delete window[cb]; }catch(_){} s.remove(); }
...
function ingest(cases,visits,src){
  DATA=(cases||[]).map(d=>({...d})); VISITS=(visits||[]).map(v=>({...v})); HAS_VISITS=VISITS.length>0;
  AVAIL=new Set(CASE_COLS.map(c=>c[1])); if(HAS_VISITS) AVAIL.add('__visits__');   // the sheet carries every column
  DATA_SRC=src; derive(); startApp();
}
async function signIn(email){
  gateErr(''); email=String(email||'').trim();
  if(!email){ gateErr('أدخل بريدك الإلكتروني.'); return; }
  const btn=$('#g-signin'); btn.disabled=true; btn.textContent='جارٍ الدخول…';
  try{
    const res=await apiCall({email});                       // READ is email-gated in the brain
    if(!res||!res.ok){
      gateErr(res&&res.error==='not-authorized'
        ? '⛔ هذا الحساب غير مُصرّح له بعرض البيانات. تواصل مع مسؤول النظام.'
        : 'تعذّر الدخول: '+((res&&res.error)||'خطأ غير معروف'));
      return;
    }
    MY_EMAIL=email; MY_ROLE=res.role||'viewer'; localStorage.setItem('gs_email',email);
    ingest(res.cases, res.visits, 'sheet');
  }catch(e){
    // offline/dev fallback: show the bundled snapshot (read-only) so the UI is still usable
    try{
      const [c,v]=await Promise.all([fetch('data/cases.json').then(r=>r.json()),fetch('data/visits.json').then(r=>r.json())]);
      MY_EMAIL=email||'(محلي)'; MY_ROLE='viewer'; gateErr('');
      ingest(c,v,'local');
      toast('تعذّر الاتصال بالمصدر المركزي — عُرضت نسخة محلية (قراءة فقط)');
    }catch(_){ gateErr('تعذّر الاتصال بالمصدر المركزي، ولا تتوفر نسخة محلية.'); }
  }finally{ btn.disabled=false; btn.textContent='دخول ›'; }
}
async function apiUpsert(rec){
  if(DATA_SRC!=='sheet'){ toast('وضع محلي (عرض فقط) — لا يُكتب في المصدر'); return; }
  try{ const res=await apiCall({action:'upsertCase', email:MY_EMAIL, row:JSON.stringify(rec)});
    toast(res&&res.ok ? '✓ حُفظ في قاعدة البيانات المركزية'
      : '⚠ لم يُحفظ: '+((res&&res.error)==='not-allowed'?'صلاحيتك للعرض فقط':((res&&res.error)||'خطأ')));
  }catch(e){ toast('⚠ تعذّر الحفظ — تحقّق من الاتصال'); }
}
async function apiDelete(id){
  if(DATA_SRC!=='sheet'){ toast('وضع محلي (عرض فقط) — لا يُحذف من المصدر'); return; }
  try{ const res=await apiCall({action:'deleteCase', email:MY_EMAIL, id});
    toast(res&&res.ok ? '✓ حُذفت من قاعدة البيانات المركزية'
      : '⚠ لم يُحذف: '+((res&&res.error)==='not-allowed'?'صلاحيتك للعرض فقط':((res&&res.error)||'خطأ')));
  }catch(e){ toast('⚠ تعذّر الحذف — تحقّق من الاتصال'); }
}
/* pull the latest from the central Sheet (catch other editors' changes) — no page reload,
 * keeps the current filters/search/page and re-checks the role. */
async function refreshFromSheet(){
  if(DATA_SRC!=='sheet'){ toast('وضع محلي — لا يوجد مصدر مركزي للتحديث'); return; }
  const btn=$('#t-refresh'); if(btn){ btn.disabled=true; btn.textContent='… تحديث'; }
  try{
    const res=await apiCall({email:MY_EMAIL});
    if(res&&res.ok){
      DATA=(res.cases||[]).map(d=>({...d})); VISITS=(res.visits||[]).map(v=>({...v})); HAS_VISITS=VISITS.length>0;
      AVAIL=new Set(CASE_COLS.map(c=>c[1])); if(HAS_VISITS) AVAIL.add('__visits__');
      MY_ROLE=res.role||MY_ROLE; derive(); applyRoleUI(); renderAll();   // renderAll (not startApp) → keeps filters/search/page
      toast('✓ حُدّثت البيانات من المصدر المركزي');
    } else toast('⚠ تعذّر التحديث: '+((res&&res.error)||'خطأ'));
  }catch(e){ toast('⚠ تعذّر الاتصال بالمصدر المركزي'); }
...
function applyRoleUI(){
  ...
  if(st) st.textContent = DATA_SRC==='sheet' ? 'المصدر المركزي' : 'نسخة محلية';
  if(sd) sd.className='dot'+(DATA_SRC==='sheet'?'':' local');
}
function applyBrand(){
...
$('#g-signin').addEventListener('click', ()=>signIn($('#g-email').value));
$('#g-email').value=MY_EMAIL;
$('#g-email').addEventListener('keydown', e=>{ if(e.key==='Enter') signIn($('#g-email').value); });
$('#t-logout').addEventListener('click', ()=>{ localStorage.removeItem('gs_email'); location.reload(); });
$('#t-refresh').addEventListener('click', refreshFromSheet);
